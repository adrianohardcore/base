-- MySQL Workbench Forward Engineering

SET @OLD_UNIQUE_CHECKS=@@UNIQUE_CHECKS, UNIQUE_CHECKS=0;
SET @OLD_FOREIGN_KEY_CHECKS=@@FOREIGN_KEY_CHECKS, FOREIGN_KEY_CHECKS=0;
SET @OLD_SQL_MODE=@@SQL_MODE, SQL_MODE='TRADITIONAL,ALLOW_INVALID_DATES';

-- -----------------------------------------------------
-- Schema mutrack_simple
-- -----------------------------------------------------
DROP SCHEMA IF EXISTS `base` ;

-- -----------------------------------------------------
-- Schema mutrack_simple
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `base` DEFAULT CHARACTER SET utf8 ;
USE `base` ;

-- -----------------------------------------------------
-- Table `tb_user`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `tb_user` ;

CREATE TABLE IF NOT EXISTS `tb_user` (
  `pk_id` INT NOT NULL AUTO_INCREMENT,
  `name` VARCHAR(120) NOT NULL,
  `email` VARCHAR(255) NOT NULL,
  `password` VARCHAR(80) NOT NULL,
  PRIMARY KEY (`pk_id`),
  UNIQUE INDEX `email_UNIQUE` (`email` ASC))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `tb_package`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `tb_package` ;

CREATE TABLE IF NOT EXISTS `tb_package` (
  `pk_id` INT NOT NULL AUTO_INCREMENT,
  `name` VARCHAR(60) NOT NULL,
  `code` VARCHAR(20) NOT NULL,
  `description` VARCHAR(150) NULL,
  `register_date` DATETIME NOT NULL,
  `owner_id` INT NOT NULL,
  PRIMARY KEY (`pk_id`),
  INDEX `fk_tb_package_tb_user_idx` (`owner_id` ASC),
  CONSTRAINT `fk_tb_package_tb_user`
    FOREIGN KEY (`owner_id`)
    REFERENCES `tb_user` (`pk_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `tb_permission`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `tb_permission` ;

CREATE TABLE IF NOT EXISTS `tb_permission` (
  `pk_id` INT NOT NULL AUTO_INCREMENT,
  `role` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`pk_id`),
  UNIQUE INDEX `role_UNIQUE` (`role` ASC))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `tb_user_permission`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `tb_user_permission` ;

CREATE TABLE IF NOT EXISTS `tb_user_permission` (
  `permission_id` INT NOT NULL,
  `user_id` INT NOT NULL,
  PRIMARY KEY (`permission_id`, `user_id`),
  INDEX `fk_tb_permission_has_tb_user_tb_user1_idx` (`user_id` ASC),
  INDEX `fk_tb_permission_has_tb_user_tb_permission1_idx` (`permission_id` ASC),
  CONSTRAINT `fk_tb_permission_has_tb_user_tb_permission1`
    FOREIGN KEY (`permission_id`)
    REFERENCES `tb_permission` (`pk_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_tb_permission_has_tb_user_tb_user1`
    FOREIGN KEY (`user_id`)
    REFERENCES `tb_user` (`pk_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


SET SQL_MODE=@OLD_SQL_MODE;
SET FOREIGN_KEY_CHECKS=@OLD_FOREIGN_KEY_CHECKS;
SET UNIQUE_CHECKS=@OLD_UNIQUE_CHECKS;

-- -----------------------------------------------------
-- Data for table `tb_permission`
-- -----------------------------------------------------
START TRANSACTION;
USE `base`;
INSERT INTO `tb_permission` (`pk_id`, `role`) VALUES (1, 'ROLE_ADMIN');
INSERT INTO `tb_permission` (`pk_id`, `role`) VALUES (2, 'ROLE_USER');

COMMIT;

CREATE TABLE IF NOT EXISTS `atendimento` (
  `id_atendimento` INT NOT NULL AUTO_INCREMENT,
  `id_tiposolicitante` INT NOT NULL,
  `id_solicitante` INT NOT NULL,
  `id_motivo` INT NOT NULL,
  `id_status` INT ,
  `id_categoria` INT ,  
  `titulo` VARCHAR(60) NOT NULL,
  `descricao` VARCHAR(200) ,
  `datacriacao` DATETIME ,
  `dataalteracao` DATETIME ,
  `id_usuariocriacao` INT ,
  `id_usuarioalteracao` INT ,
  `id_usuarioresponsavel` INT ,
  PRIMARY KEY (`id_atendimento`),
  INDEX `fk_tb_atendimento_tb_motivo_idx` (`id_motivo` ASC),
  INDEX `fk_tb_atendimento_tb_categoria_idx` (`id_categoria` ASC)
)
ENGINE = InnoDB;


CREATE TABLE IF NOT EXISTS `motivo` (
  `id_tiposolicitante` INT NOT NULL AUTO_INCREMENT,
  `descricao` VARCHAR(60) NOT NULL,
  `datacriacao` DATETIME ,
  `dataalteracao` DATETIME ,
  `id_usuariocriacao` INT ,
  `id_usuarioalteracao` INT ,
  PRIMARY KEY (`id_tiposolicitante`)
)
ENGINE = InnoDB;

CREATE TABLE IF NOT EXISTS `categoria` (
  `id_categoria` INT NOT NULL AUTO_INCREMENT,
  `descricao` VARCHAR(60) NOT NULL,
  `datacriacao` DATETIME ,
  `dataalteracao` DATETIME ,
  `id_usuariocriacao` INT ,
  `id_usuarioalteracao` INT ,
  PRIMARY KEY (`id_categoria`)
)
ENGINE = InnoDB;



